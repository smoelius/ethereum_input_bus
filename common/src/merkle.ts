...
export function kec256_assemble_tree(height: number, offset: number, file: Buffer): BN[] {
  ...
  if (height === 0) {
    ...
    const root = new BN(Hasher.keccak256(Buffer.concat([
...
export function extract_data(file: Buffer, start: number, end: number): BN[] {
...
export function calculate_proof_length(start: number, end: number, file_length: number): number {
  ...
  if (file_length > 0 && start < end) {
    const height = math.ceil_log2_big(math.ceil_div_big(new BN(file_length), new BN(32)))
...
export function extract_proof(start: number, end: number, file_length: number, merkle_tree: BN[]):
    BN[] {
  let proof: BN[] = []
  if (file_length > 0 && start < end) {
    const height = math.ceil_log2_big(math.ceil_div_big(new BN(file_length), new BN(32)))
...
function _extract_proof(height: number, offset: number, start: number, end: number,
    file_length: number, tree_index: number, merkle_tree: BN[]): [BN[], number] {
  const width = 2 ** height * 32
  if (height === 0) {
